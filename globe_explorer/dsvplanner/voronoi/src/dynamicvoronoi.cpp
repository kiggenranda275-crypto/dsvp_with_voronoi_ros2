...
{
    gridMap = _gridMap;
    initializeEmpty(_sizeX, _sizeY, false); // 使用空地图初始化
    ...
    for (int x = 0; x < sizeX; x++)
    {
        for (int y = 0; y < sizeY; y++)
        {
            ...
            {
                dataCell c = data[x][y];
                if (!isOccupied(x, y, c)) // 如果占用
                {
                    bool isSurrounded = true; // 观察四周是否是被包围， 上下左右四个方向，如果全部为1，则表示被包围
...
std::vector<std::vector<RGB>> DynamicVoronoi::get_visual()
{
    std::vector<std::vector<RGB>> vis_map;
    vis_map.resize(sizeX, std::vector<RGB>(sizeY, RGB()));
    for (int y = sizeY - 1; y >= 0; y--)
    {
        for (int x = 0; x < sizeX; x++)
        {
    ...
    return vis_map; // 返回可视化数组地图，RGB等级的
}
