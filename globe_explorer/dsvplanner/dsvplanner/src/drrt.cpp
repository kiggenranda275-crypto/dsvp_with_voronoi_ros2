...
dsvplanner_ns::Drrt::Drrt(rclcpp::Node::SharedPtr& node_handle, octomanager* manager, DualStateGraph* graph,
...
bool dsvplanner_ns::Drrt::inSensorRange(StateVec& node)
{
    ...
    for (int i = 0; i < localThreeFrontier_->points.size(); i++)
    {
        ...
        if (manager_->getCellStatusPoint(node) == CellStatus::kFree)
        {
...
// 全局规划阶段使用，使用最近的全局边界点作为下一个节点，论文算法3
void dsvplanner_ns::Drrt::getNextNodeToClosestGlobalFrontier()
{
    ...
    nextNodeFound_ = false;
    for (int i = dual_state_graph_->local_graph_.vertices.size() - 1; i >= 0; i--) // //从尾到头搜索。最大的节点搜索图
        // 索引大的节点意味着它们是最新的节点，更靠近机器人位置
    {
        ...
        for (int j = 0; j < dual_state_frontier_->global_frontier_->size(); j++) // 从最大的节点遍历全局边界
        {
            ...
            if (CellStatus::kOccupied == manager_->getLineStatusBoundingBox(p1, p2, params_.boundingBox))
            {
                continue; // 仅当节点之间没有被占用的体素时
            }
            else
            {
                NextBestNodeIdx_ = i;
...
// 迭代Tree，对新的进行配置，添加进Tree
void dsvplanner_ns::Drrt::plannerIterate()
{
    // RCLCPP_INFO(nh_->get_logger(), "Planner Iterate");
    ...
    while (!candidateFound)
    {
        ...
        if (((double)rand()) / ((double)RAND_MAX) > 0.75 && localThreeFrontier_->size() > 0) // RRT扩展范围
        {
            if (local_plan_ == true)
            {
                generateNodeArroundFrontierSuccess = generateRrtNodeToLocalFrontier(newState);
            }
            if (!generateNodeArroundFrontierSuccess) // 生成新的RRT节点
    ...
    // RCLCPP_INFO(nh_->get_logger(), "New State: %f, %f, %f", newState[0], newState[1], newState[2]);
    ...
    // RCLCPP_INFO(nh_->get_logger(), "Direction: %f, %f, %f", direction[0], direction[1], direction[2]);
    ...
    else
    {
        ...
        { // connection is free
            ...
            // RCLCPP_INFO(nh_->get_logger(), "New Node: %f, %f, %f", newState[0], newState[1], newState[2]);
            dual_state_graph_->addNewLocalVertexWithoutDuplicates(p1, dual_state_graph_->local_graph_); // 添加新的无重复顶点，添加到局部图中
...
{
    RCLCPP_INFO(nh_->get_logger(), " Planner Init");
    kdTree_ = kd_create(3);
    ...
    global_vertex_size_ = 0;
    geometry_msgs::msg::Pose p1;
    if (global_plan_ == false) // 局部规划阶段
    {
        std::cout << "\033[34m Exploration Stage \033[0m" << std::endl;
        rootNode_->state_ = root_; // 根节点为当前位置，机器人当前位置；调用odomcallback
        kd_insert3(kdTree_, rootNode_->state_.x(), rootNode_->state_.y(), rootNode_->state_.z(), rootNode_);
        iterationCount_++;

        if (remainingLocalFrontier())
        {
            RCLCPP_INFO(nh_->get_logger(), "Remaining Local Frontier");
            localPlanOnceMore_ = true;
            loopCount_ = params_.kLoopCountThres;
            normal_local_iteration_ = true;
            keepTryingNum_ = params_.kKeepTryingNum; // Try 1 or 2 more times even if there no local frontier
            remainingFrontier_ = true;
    ...
    {
        std::cout << "\033[32m Relocation Stage \033[0m" << std::endl;
        localPlanOnceMore_ = true;
        StateVec node1;
        double gain1;
        if (dual_state_graph_->global_graph_.vertices.size() > 0) // 全局拓扑图存在
        {
            node1[0] = dual_state_graph_->global_graph_.vertices[0].location.x; // 第一个节点，便是最靠近当前位置的全局节点
            ...
            dual_state_graph_->execute();
            getNextNodeToClosestGlobalFrontier(); // 寻找最近的全局边界对应的
            if (nextNodeFound_)                   // 如果找到了需要寻找的全局边界点
            {
                ...
                RCLCPP_INFO(nh_->get_logger(), "The best node is %d  重定位阶段找到", NextBestNodeIdx_);
            }
            else
            {
...
void dsvplanner_ns::Drrt::pruneTree(StateVec root) /// 生成RRT tree
{
    // 机器人位置添加进RRG中
    dual_state_graph_->pruned_graph_.vertices.clear();
    geometry_msgs::msg::Pose p1;
    ...
    StateVec node;                                                             // 遍历
    for (int i = 1; i < dual_state_graph_->pruned_graph_.vertices.size(); i++) // 遍历拓扑图的节点
    {
    ...
    RCLCPP_INFO(nh_->get_logger(), "Remaining node count: %d", remainingNodeCount_);
}

// 视点增益；由octomap对应的点能否看到实现；
double dsvplanner_ns::Drrt::gain(StateVec state)
{
    ...
    // 使用三重循环遍历以当前点为中心、半径为 kGainRange 的立方体内的所有离散点：
        ...
        vec[0] += disc)
    {
            ...
            vec[1] < std::min(state[1] + params_.kGainRange, maxY_); vec[1] += disc)
        {
            ...
            {
                StateVec dir = vec - origin; // 点与机器人的距离
                ...
                if (fabs(dir[2] < sqrt(dir[0] * dir[0] + dir[1] * dir[1]) * tan(M_PI * params_.sensorVerticalView / 360)))
                {
                    insideAFieldOfView = true; // 在机器人视角内
                }
                ...
                // 计算前沿点增益
                for (int i = 0; i < dual_state_frontier_->local_frontier_->size(); i++)
                {
                    double dis = sqrt(SQ(dual_state_frontier_->local_frontier_->points[i].x - vec[0]) +
                        SQ(dual_state_frontier_->local_frontier_->points[i].y - vec[1]));
                    if (dis < 3.0)
                    {
                        gain += params_.kGainFrontier;
                        break;
                    }
                }

                double probability;
                CellStatus node = manager_->getCellProbabilityPoint(vec, &probability);
                if (node == CellStatus::kUnknown)
                {
                    if (CellStatus::kOccupied !=
                        this->manager_->getVisibility(origin, vec, false))
                    {
                        gain += params_.kGainUnknown; // 能够看见未知的区域；增益++
                    }
                }
                else if (node == CellStatus::kOccupied) // 该点为占用
                {
                    if (CellStatus::kOccupied !=
                        this->manager_->getVisibility(origin, vec, false))
                    {
