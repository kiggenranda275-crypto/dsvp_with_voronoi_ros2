...
#include "dsvplanner/voronoi.hpp"
#include "nav_msgs/msg/occupancy_grid.hpp"
#include <cmath>
#include "dsvplanner/octo.hpp"
...
namespace dsvplanner_ns
{
    ...
    class DualStateFrontier
    {
        ...
        typedef message_filters::Synchronizer<syncPolicy> Sync; // ros消息订阅多个传感器，用于同步信息
        ...
        // 更改TODO
        std::shared_ptr<Voronoi> voronoi_local_;
        std::shared_ptr<Voronoi> voronoi_global_;
        void globalVoronnoiFrontierExecuter(); // 计算局部Voronoi
        void getGlobalVoronoiForntier();       // 计算全局Voronoi
        Eigen::Vector3d getRealPosition(int i, int j, bool is_local = false);
        int voronoi_loop;
        pcl::PointCloud<pcl::PointXYZ>::Ptr voronoi_local_forntier =
            pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());

        pcl::PointCloud<pcl::PointXYZ>::Ptr voronoi_global_frontier =
            pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());

        rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr voronoi_local_pub_;
        rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr voronoi_global_pub_;
        int kVoronoiUnknownNum;
        void isFrontier(StateVec point);
        pcl::PointCloud<pcl::PointXYZ>::Ptr VoronoiEdgesToFrontier(std::vector<Eigen::Vector3i>& edges, bool is_local = false);
        void get2Dindex(StateVec point, int& i, int& j, bool is_local = false);
        octomanager* manager_;
        void getLoaclVoronoiForntier();
        rclcpp::TimerBase::SharedPtr global_forniter_timer_;
        bool IsPointFrontier(StateVec point, bool is_local = false);

        public:
        DualStateFrontier(rclcpp::Node::SharedPtr& node_handle, octomanager* manager);
        bool readParameters();
        bool initialize();
        void execute();
        ~DualStateFrontier();
